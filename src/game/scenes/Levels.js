...
// ─────────────────────────────────────────────────────────────
//
// Player score determines their employee rank.
// The game can use this for:
// - Level progression
// - Employee title
// - Difficulty
// - Unlocks
...
export function getNextEmployeeLevel(score = 0) {
    const currentLevel = getEmployeeLevel(score);
...
export function getPointsToNextLevel(score = 0) {
    const currentLevel = getEmployeeLevel(score);
    const nextLevel = getNextEmployeeLevel(score);
    ...
    return Math.max(
        0,
        nextLevel.minScore - Number(score)
    );
}


// ─────────────────────────────────────────────────────────────
// PROGRESS THROUGH CURRENT EMPLOYEE LEVEL
// ─────────────────────────────────────────────────────────────

export function getLevelProgress(score = 0) {
    const currentLevel = getEmployeeLevel(score);

    // MASTER has no upper limit
    ...
    const progress =
        (Number(score) - currentLevel.minScore) /
        range;
